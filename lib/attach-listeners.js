...
function attach(ziggy) {
  ziggy.client.on('registered', ziggy.emit.bind(ziggy, 'ready'))

  ziggy.client.on('message#', function (nick, to, text, message) {
    var user = ziggy.user(nick)

    ziggy.emit('message', user, to, text)
  })

  ziggy.client.on('pm', function (nick, text, message) {
    var user = ziggy.user(nick)

    if(ziggy.settings.users[nick] &&
  ...
  ziggy.client.on('nick', function (oldnick, newnick, channels) {
    ...
    for(var i = 0, l = channels.length; i < l; ++i) {
      ...
      if(!ziggy.settings.channels[channel].users[oldnick]) continue

      ziggy.settings.channels[channel].users[newnick] = Object.create(
          ziggy.settings.channels[channel].users[oldnick]
      )
  ...
  ziggy.client.on('invite', function (channel, from, message) {
